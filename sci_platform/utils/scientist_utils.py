        ...
        if filename.endswith(".txt"):  
            ...
            try:
                ...
                with open(file_path, 'rb') as file:
                    ...
                    encoding = encoding_result['encoding'] or 'utf-8'  # Default to UTF-8 if detection fails
                ...
                try:
                    file_dict_old = eval(file_content)  # Avoid eval for untrusted input
                    file_dict = {
                        'title': file_dict_old.get('title'),
                        'abstract': file_dict_old.get('abstract'),
                        'year': file_dict_old.get('year'),
                        'citation': file_dict_old.get('citation'),
                        'id': id,
                        'authors': None,
                        'cite_papers': None
                    }
                ...
                except (SyntaxError, KeyError) as parse_error:
                    print(f"File {filename} content could not be parsed. Skipping. Error: {parse_error}")
                    continue
